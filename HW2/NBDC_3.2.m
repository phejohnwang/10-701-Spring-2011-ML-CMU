...
% Question 3.2~3.3 by Pheno @ Gatech ECE
%% Reading Data
clear all;
...
% The definition of confusion matrix in homework (i - predicted, j - truth) is different from wiki (i - truth, j - predicted)
...
for k = 1:length(label)
	accuracy(k) = confusion(k,k)/sum(confusion(k,:));
end
accuracy
